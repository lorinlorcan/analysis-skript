...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\addplot[color1, smooth, samples=100, domain=0:8] {0.5*(x-1)^3 + 1.5};
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\addplot[color1, smooth, samples=100, domain=0:25000] {-60*x+90000};
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\addplot[name path=k, color1, smooth, samples=100, domain=0:4] {(x - 1.5)^3 + (x-2)^2 +1};
			\addplot[name path=e, mkred, smooth, samples=100, domain=0:4] {x};
			\addplot[color2, smooth, samples=100, domain=0:4] {1};
			...
			\node[above, color2] at (3, 1) {\( p(x) \)};
			\node[below, mkred] at (3, 3) {\( E \)};
			\node[above, color1] at (2.9, 3.2) {\( K \)};
		\end{axis}
	\end{tikzpicture}
	\quad
	\begin{tikzpicture}
		\begin{axis}[
			...
			\addplot[color2, smooth, samples=100, domain=0:4] {-(2.2 / 2.81) * x + 2.2};
			...
			\node[left, color2] at (0, 2.2) {\( P \)};
			\node[above, color2] at (3, 0) {\( p(x) \)};
